...
> Governing Framework: `02_Framework.md` v1.4  
...
這份 Playbook 是部門級的 **Golden Reference**。它不是另一份治理文件，而是工程師開始工作時可以直接照著走的入口。

每項工作只需要先回答五個問題：

1. **Work Level**：這是 Product/Program、Epic、Feature，還是 PBI/User Story？
2. **Archetype**：Greenfield、Legacy Modernization/Migration，還是 Standard Delivery？
3. **Current Stage**：Research、Design、Plan、Implement、Validate？
4. **Control Profile**：System Criticality、Change Risk Tier、AI Execution Mode 是什麼？
5. **Next Move**：現在應使用哪個 skill，留下什麼 artifact，通過哪個 gate？

Golden Flow 保留五個直覺入口，但內部使用前兩個 live sessions 已確認的完整能力鏈：

```text
Research                  Design               Plan    Implement   Validate
(Understand→Explore→Define) → (Design→Challenge) → Plan → Implement → (Verify→Release handoff)
```

**核心規則：先完成當前 stage 的 minimum artifact 與 gate，再進下一步；不要一次啟動所有 skills。**
...
### 2.1 Flow 與 Framework 的對應

| Golden Stage | Framework Lifecycle | Engineer 的核心問題 |
|---|---|---|
...
| Plan | Design → Build handoff | 如何先切 P0 vertical slices，再按需將單一 P0 切成 execution tasks？ |
| Implement | Build | 如何依 plan 以 TDD/controlled change 完成？ |
| Validate | Verify | Requirement、risk、quality 是否有 fresh evidence？ |
...
```text
Input → Lead Skill → Activity → Artifact → Human Gate
```
...
Skill name 是 department capability alias；不同 agent/tool 可以有不同 implementation mapping。
...
| Lead skills | 依主要未知選 `understand-anything`、`codebase-research`、`grill-me` 或 `/opsx:explore`；bug/incident 加 `systematic-debugging` |
...
| Lead capability | `system-design` bundle：evidence skills → `/opsx:explore` → `superpowers:brainstorming` → `grill-with-docs` |
...
| Lead skills | `superpowers:brainstorming`；需要 durable change package 時用 `/opsx:propose` 或 `/opsx:new` + `/opsx:continue`；完成後用 `grill-with-docs` |
...
| Planning Depth | Input | Lead Skill | Minimum Artifact | Gate |
|---|---|---|---|---|
...
| Lead skills | OpenSpec change 先 `/opsx:apply`；執行紀律使用 `using-git-worktrees` → `test-driven-development` → `executing-plans` 或 `subagent-driven-development` |
...
| Lead skills | `requesting-code-review`、`receiving-code-review`、`verification-before-completion`；Expanded OpenSpec 加 `/opsx:verify` |
...
| Stage | Lead Skills | Key Activities | Required Artifact | Human Gate |
|---|---|---|---|---|
...
1. **不要每個 P0 都跑全部 skills。** 只使用當前 stage 的 lead skill 與必要 supporting skill。
...
## Current Stage and Lead Skill

## Expected Artifact and Human Gate
```

## Appendix B — Minimal Validation Record
...
- `04_Framework_Overview`：呈現 Framework + Golden Flow 的整體關係。
- `05_Decision_Tree`：依 Work Level、Archetype、Risk、Execution Mode、Current Stage 導向 skill/artifact/gate。
