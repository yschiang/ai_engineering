...
> Governing Framework: `02_Framework.md` v1.9
...
進入新的 work item 時，先用 Six Golden Questions 完成一次 initial routing：
...
4. **Current Stage**：目前最大的未知或風險屬於 Research、Design、Plan、Implement 或 Validate？
5. **Control Profile**：System Criticality、Change Risk Tier 與 AI Execution Mode 是什麼？
6. **Next Move**：下一個 required capability、minimum evidence 與 gate decision 是什麼？

這六題是 **initial routing**，都要判斷。進入執行後不必每一步重填：通常只更新 Current Stage 與 Next Move；scope、architecture、risk 或 AI authority 改變時，再完整重做六題。
...
### Step 1 — 完成 Six Questions Initial Routing
...
| Durable Change：OpenSpec `tasks.md` 尚未達 executable plan standard | 依 `writing-plans` quality contract 補強 `tasks.md`；只有 approved bridge 才同時 invoke upstream skill | Exact files/interfaces/tests/commands 可直接執行 | OpenSpec `tasks.md`；不建立第二份 plan |
...
**不要按品牌選 skill，也不要預設依序跑 `grill-me → explore → brainstorming → OpenSpec`。** 要把人腦中的決定問清楚，用 `grill-me`；還不知道要不要改、怎麼改，用 `/opsx:explore`；已決定要改但方案未定，用 `brainstorming`；需要讓後續 session／人／AI 依同一份約定繼續工作，才用 OpenSpec 保存 why／what／how／tasks。

P0／P1 的快速選擇：

| 要做什麼已清楚？ | 後續需要跨 session／AI／Engineer 接手？ | Route |
|---|---|---|
| **否** | 尚未判斷 | 先處理未知：系統不清楚用 research、人的決定不清楚用 `grill-me`、change/options/scope 不清楚用 `/opsx:explore` |
| **是** | **否** | **直接使用 Superpowers**：按需要完成 short design／`brainstorming` → inline plan 或 `writing-plans` → TDD → `requesting-code-review`／`receiving-code-review` → verification |
| **是** | **是** | **OpenSpec + Superpowers execution skills**：OpenSpec 保存 why／what／how／tasks；TDD、`requesting-code-review`／`receiving-code-review`、verification 負責執行品質；design/plan 只保留一份 SSOT |

Team 應依既有 workflow 選定 default profile，避免每張 ticket 重新爭論工具：

| Team Default Profile | 適用情況 | Team 要先約定什麼 |
|---|---|---|
| **Fast Delivery** | 多數 P0／bounded P1 已清楚，ticket + PR 足以交接 | Superpowers/equivalent 的 design、plan、TDD、review、verification 放在哪裡 |
| **Durable Change** | 多人、多 session／AI、重要 behavior contract，或未來需要追查設計理由 | OpenSpec Change scope、branch/PR review、owner、archive timing；TDD/review/verification 如何銜接 |
| **Discovery First** | Brownfield、需求模糊、root cause 或方案未明 | Research／`grill-me`／`/opsx:explore` 的使用順序與何時轉入 Fast Delivery 或 Durable Change |

Team profile 是**全隊共同約定**，不是每位 Engineer 的臨時偏好。選擇時看既有 ticket/PR/ADR 流程、工具可用性、團隊熟悉度、交接頻率與誰能長期維護 artifacts。客觀的 architecture/risk/evidence trigger 仍優先；Team 可以替換工具，但 required capability、output、Human Gate 與 evidence quality 不可降低。

### Step 3 — 執行中只重新判斷有變化的 Context
...
|---|---|
| OpenSpec | 保存可交接、可維護的 change agreement：proposal、specs、design、tasks、implementation alignment、archive |
| Research capabilities | 建立可信 system/domain/code understanding，提供 proposal/design 的 evidence basis；manual defaults 不需安裝 |
...
OpenSpec 與 Superpowers 不是二選一，但要先決定誰擁有 design/plan artifact：

- **OpenSpec 保存工程上下文**：把 why／what／how／tasks 留下來，讓下一個 session、AI 或 Engineer 能接著做，不必重新解釋。
- **Superpowers 規範工程動作**：用 `brainstorming` 做設計、`writing-plans` 做 P0 implementation plan、TDD 實作、code review 與 verification 收尾。

實際使用 upstream Superpowers 時，`brainstorming` 會要求建立 `docs/superpowers/...` design，不能假裝它已自動寫入 OpenSpec。Team 採 Durable Change profile 時應選一種明確做法：

1. 使用經 Team review 的 OpenSpec custom schema／bridge，把兩套輸出整合成同一 SSOT；或
2. 由 OpenSpec 擁有 proposal/specs/design/tasks，Superpowers 只使用不重複產生 design/plan 的 execution skills（TDD、review、verification），並以相同 quality contract review OpenSpec artifacts。

未核准整合前，不同時維護 OpenSpec design/tasks 與 `docs/superpowers/...` 的兩份內容。未使用 OpenSpec 時，Superpowers output 留在 Team 既有 design/plan/PR chain。

對「要做什麼已經清楚」的 bounded P0／P1，Superpowers 通常是最快的 default path；design 可以依複雜度縮短，但不能跳過 target behavior、constraints、test direction 與 Human approval。只有需要保存並交接工程上下文時，才再加 OpenSpec。

是否使用 OpenSpec，不看「個人還是大團隊」，而看這次 change 是否要跨 session／AI／Engineer 交接、是否需要長期維護 behavior contract，以及未來是否要追查當時的設計理由。簡單、低風險、單一 owner 的 P0 可以只用 ticket + PR；需要保存工程上下文的 P1/P0 才建立 scoped OpenSpec Change。

`/opsx:verify` 檢查 implementation 與 proposal/specs/design/tasks 的 alignment；CLI `openspec validate` 檢查 artifact structure/consistency。**兩者都不能取代 TDD、code review、runtime tests、NFR evidence 或 release readiness。**

`grill-me` 與 `/opsx:explore` 都不負責保存正式工程上下文。重要決定若要交接、長期維護或稽核，必須在討論後寫回既有 work item／design SSOT，或交給 OpenSpec。
...
```text
manual research / grill-me only for the first question that is still unclear
→ /opsx:explore only when problem/options/scope are still fuzzy
→ Feature/Detailed Design using brainstorming when approach is not approved
→ decide where to keep the result: existing design SSOT, or scoped OpenSpec when the context must survive handoff
→ grill-with-docs when an artifact needs adversarial challenge
→ approve P1 design/agreement
→ to-tickets: P0 vertical slices + Blocked by
→ keep P0 set in tracker; reference it from OpenSpec tasks.md when used
→ for each frontier P0: writing-plans when triggered
→ TDD + execution skills; /opsx:apply is the entry when OpenSpec is used
→ code review + verification
→ /opsx:sync → /opsx:archive when OpenSpec is used
```

**Complex / Brownfield Path**：

```text
select system-research / codebase-research / grill-me by the first question that is still unclear
→ /opsx:explore only if change intent/options/scope remain unclear
→ System Design when triggered; otherwise Feature/Detailed Design using brainstorming discipline
→ keep the result in existing architecture/design SSOT, or scoped OpenSpec when later sessions/people must continue from it
→ grill-with-docs + Human review when needed
→ approve P1 design/agreement
→ to-tickets: P0 tracer bullets + dependency frontier
→ keep P0 set in tracker; reference it from P1 OpenSpec tasks.md when used
→ JIT executable tasks using writing-plans quality
→ TDD + bounded execution; /opsx:apply is the entry when OpenSpec is used
→ review + /opsx:verify
→ /opsx:sync → /opsx:archive when OpenSpec is used
```
...
1. **OpenSpec 保存可交接的 change context**：proposal 說 why、specs 說 what、design 說 how、tasks 記錄 delivery references 或 execution steps，archive 回到 source of truth。
...
3. **Superpowers 規範工程動作**：Feature/Detailed Design、JIT implementation planning、TDD、bounded execution、review、fresh verification；結果仍寫回選定的 SSOT。
...
| Golden default skills | Primary discipline：`using-git-worktrees` → `test-driven-development` → `executing-plans` 或 `subagent-driven-development`；採 OpenSpec 時由 `/opsx:apply` 讀取 tasks 並執行相同 TDD discipline |
...
最容易混淆的四個 capabilities 先用下表分流；詳細 quick selector 見 §1 Step 2。
...
| 讓下一個 session、AI 或 Engineer 依同一份 why／what／how／tasks 繼續工作 | OpenSpec `/opsx:propose` or `new/continue` | 建立可維護的 Change；OpenSpec 不負責 TDD、code review 或 runtime tests |
...
→ worktree + TDD + execute tasks;
  OpenSpec change uses /opsx:apply as the task entry
...
- Stage 只定位問題類型；第一個還沒搞清楚的問題決定下一個 skill。`grill-me` 用來訪談，`/opsx:explore` 用來探索，`brainstorming` 用來形成 approved design，OpenSpec 用來保存可交接的 change context；不呈現為固定 chain。
- OpenSpec 保存 why／what／how／tasks；Superpowers 提供 design／plan／TDD／review／verification discipline。一起使用時必須先核准 artifact integration；否則由 OpenSpec 擁有 design/tasks，只搭配不重複建文件的 TDD／review／verification skills。
