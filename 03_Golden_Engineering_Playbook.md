...
本表列的是 Department **Golden defaults**。System Design 等工程活動仍由 Engineer／Architect 執行；installable skill entries 則依 Registry 的 approved mapping 使用。Team 採用 equivalent skill 時，必須接受相同 required inputs、產出相同 minimum artifact、遵守相同 stop condition、通過相同 gate，並保留相同 evidence quality。
...
這張 Matrix 呈現 Department Golden activities 與 skills。`Primary` 表示該情境的預設安全路徑，不代表每個 work item 都要執行所有列；由 Six Golden Questions 與 Trigger Rules 選出下一個 engineering activity 或 supporting skill。System Design 沒有 installation／invocation；其他 installable entries 依 Skill Registry 使用 approved mapping。
...
- `guides/05_Decision_Tree.md`：開頭提供 compact router，再依 Six Golden Questions 導向 required engineering activity／supporting skill、artifact/gate。
...
- 首先問 existing work location 與最大未知/風險，再選 required engineering activity／supporting skill、artifact placement 與 gate。
