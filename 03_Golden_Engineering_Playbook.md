...
工程師先從 §1 選 Fast Delivery 或 Complex/Durable Change。新 work item 仍要確認 existing record、Work Level、Archetype、Current Stage、Control Profile 與 Next Move，但使用 Team 已有資訊即可，不建立新表單；需要詳細 routing 時再看 §1.3–1.5。
...
## 1. Engineer Start Here — Choose One Delivery Route

工程師日常先做一個選擇即可：**這次 change 是否需要一份會跨 session／Engineer／AI 延續的 spec？**

| 你的工作 | 使用方式 | 主路徑 |
|---|---|---|
| **Fast Delivery**：要做什麼清楚、bounded P0／P1、ticket + PR 足以延續 | **Superpowers end-to-end** | design as needed → plan as needed → worktree → TDD → code review → fresh verification |
| **Complex / Durable Change**：多人、多 session／repo、重要 behavior/contract，或 spec/tasks 必須持續更新與追查 | **OpenSpec 管 spec/tasks；Superpowers 管每個 task 的 TDD/review discipline** | OpenSpec proposal/specs/design/tasks → Change Gate → `/opsx:apply` → per-task TDD → review/verify → sync/archive |
| **還不知道要做什麼** | 先處理第一個未知，再回來選上面兩條 | research、`grill-me`、`/opsx:explore` 或 `systematic-debugging` 擇一；這不是第三條 delivery flow |

### 1.1 Fast Delivery — Let Superpowers Finish the Change

```text
ticket + acceptance
→ brainstorming only when design is not already clear
→ writing-plans only when execution is multi-step/complex
→ using-git-worktrees
...
這條路徑不需要 OpenSpec。Superpowers design/plan 與既有 ticket／PR chain 就是 SSOT。

### 1.2 Complex / Durable Change — Connect OpenSpec to Superpowers TDD

```text
OpenSpec proposal + specs + design + tasks
→ Human approves Change Gate
→ create isolated worktree
→ start /opsx:apply for the selected Change
→ for each unchecked OpenSpec task:
     use superpowers:test-driven-development
     RED: write one failing test and observe the expected failure
     GREEN: make the smallest implementation pass
     REFACTOR: improve safely while tests stay green
     run targeted tests/typecheck
     update the same OpenSpec task only after evidence passes
→ requesting-code-review → receiving-code-review
→ verification-before-completion + /opsx:verify
→ Human Evidence Gate
→ /opsx:sync → /opsx:archive
```

在 apply session 中可以直接給 Agent 這段 execution contract：

```text
Apply the selected OpenSpec Change. Treat its specs, design and tasks as the only implementation SSOT.
For each unchecked task, use superpowers:test-driven-development: observe the correct failing test,
make the minimum implementation pass, refactor only while green, and run targeted verification.
Do not mark a task complete without fresh evidence. If implementation requires a spec, design or scope
change, stop and use /opsx:update; recheck the Change Gate based on scope/risk.
Do not create another design, plan or task ledger.
```

連接點只有一個：**`/opsx:apply` 依 `tasks.md` 決定做哪個 task；Superpowers TDD 決定該 task 如何實作。** Durable Change 不使用 Superpowers `writing-plans`，也不建立另一個 plan 或 task ledger。

### 1.3 Confirm Minimum Routing Context — Reuse Existing Records
...
| Design | 是否需要 System Design？要改成什麼、為什麼選這個方案？ | System Design when triggered / Fast Delivery 用 `superpowers:brainstorming` / Durable Change 用 OpenSpec proposal/specs/design / `grill-with-docs` |
| Plan | 是把 P1 切成哪些 P0，還是把單一 P0 切成哪些 execution tasks？ | `to-tickets`；Fast Delivery 按需用 `superpowers:writing-plans`；Durable Change 使用 OpenSpec `tasks.md` |
...
| Fast Delivery：已決定要新增／改變 behavior，但 target behavior、approach 或 trade-offs 未 approved | `superpowers:brainstorming` | Selected design 已寫下並由 Human Owner approval | Upstream `docs/superpowers/...`，或 Team 已核准的等價 design SSOT |
...
| Durable Change：direction 已清楚，而且下一個 session、AI 或 Engineer 必須接著同一份 why／what／how／tasks 工作 | `/opsx:propose`；複雜 change 用 `/opsx:new` + `/opsx:continue` | Proposal、specs、design、tasks 已足以 review 和接續執行 | OpenSpec Change；套用相同 design quality contract，不另跑會產生第二份文件的 skill |
| Approved P1 尚未切成 sprint-ready slices | `to-tickets` | P0 tracer bullets 可獨立驗證，且 blockers/frontier 清楚 | Existing tracker；由 P1 OpenSpec `tasks.md` reference when used |
| Fast Delivery：單一 P0 即將執行，且 multi-step/high-risk/complex | `superpowers:writing-plans` | Exact files/interfaces/tests/commands 可直接執行 | Upstream plan output，或 Team 已核准的等價 plan SSOT |
| Durable Change：執行到的 OpenSpec task 顆粒過粗 | 在 `/opsx:apply` 當下直接於 `tasks.md` 展開巢狀子項 | 每個子項列 exact files、failing test、green criterion | Engineer 快速確認後繼續；不呼叫 `writing-plans`、不另開文件 |
...
未知關閉後，回到 §1 的兩條 delivery routes；不要把 discovery capabilities 串成固定流程。Team 可以選 approved equivalent，但不能降低 architecture/risk trigger、TDD/test/review、Human Gate 或 evidence quality。
...
### 2.2 OpenSpec and Superpowers — One Operational Rule

| Route | Spec／plan owner | Implementation rule |
|---|---|---|
| **Fast Delivery** | Superpowers output + existing ticket／PR chain | Superpowers end-to-end |
| **Complex / Durable Change** | OpenSpec proposal／specs／design／tasks | `/opsx:apply` selects and tracks the task；Superpowers TDD/review disciplines control how code is changed |

> **OpenSpec 決定做什麼並保存 agreement；Superpowers TDD 決定每個 task 怎麼安全實作。**

不用比較兩套 framework 的功能清單。只要遵守兩條規則：同一 change 只有一份 spec/design/plan/task ledger，且 automation 只提供 evidence，Change Gate／Evidence Gate 仍由 Human Owner 決定。
...
| **`superpowers:writing-plans`** | Fast Delivery 的單一 P0 即將執行，且 multi-step、high-risk 或 complex | JIT executable plan：exact files/interfaces、steps、tests、commands、evidence | Plan 可由沒有背景的 Engineer 執行；無 unresolved design decision |
...
→ Fast Delivery: writing-plans（per P0、JIT、risk/complexity triggered）
  Durable Change: OpenSpec tasks.md（single ledger；粗 task 在 /opsx:apply 就地展開）
...
- Fast Delivery 的簡單、低風險、單一步驟 P0 可跳過完整 `writing-plans`，但仍保留 acceptance、test direction 與 evidence。
...
| **`superpowers:executing-plans`** | Fast Delivery 已有 plan，需要在另一 session 分批執行 | Completed plan batches + review checkpoints | 每批 plan items 與 evidence completed |
| **`superpowers:subagent-driven-development`** | Fast Delivery plan tasks mostly independent，可在同一 session 分工 | Per-task implementation + spec/quality review | 每 task 通過 review，最後 whole-change review |
...
| **Fast Delivery implementation planning** | 單一即將執行的 P0 + repository context | Triggered `superpowers:writing-plans` | Exact files/interfaces/tests/commands 的 executable plan | 每個 task 可獨立理解、test、review；無 unresolved design decision |
| **Durable Change implementation planning** | Approved OpenSpec Change | OpenSpec `tasks.md` only | Single task ledger；粗 task 在 `/opsx:apply` 時就地展開 | Engineer 快速 checkpoint；design change 另走 `/opsx:update` |

Typical existing integration：delivery decomposition 通常發生於 backlog refinement／iteration planning，P0 set 與 `Blocked by` 留在 tracker。Fast Delivery plan 留在單一 P0 或 Superpowers plan；Durable Change 只使用 OpenSpec `tasks.md`。Product/Engineering Owner 對 P0 outcome/dependency accountable，Change Owner 對 executable tasks accountable。正常情況不新增 planning ceremony；只有 dependency、risk 或 design ambiguity 需要既有 owner/reviewer 介入。
...
Fast Delivery implementation planning 採 **just-in-time**。簡單、低風險、單一步驟 P0 可用 inline plan，跳過完整 `writing-plans`。需要完整 plan 時，每個 task 至少包含：
...
Durable Change 不呼叫 `writing-plans`。OpenSpec `tasks.md` 是唯一 task ledger；顆粒過粗時依 §4.5 在原檔就地展開。
...
| Item | Golden Standard |
|---|---|
| Inputs | Fast Delivery approved plan，或 Durable Change approved OpenSpec `tasks.md`；isolated workspace、baseline tests |
| Golden default skills | Fast Delivery：`using-git-worktrees` → `test-driven-development` → selected Superpowers execution skill；Durable Change：isolated workspace → `/opsx:apply` 讀取唯一的 `tasks.md`，並套用 TDD/debugging discipline；不使用 `writing-plans` 或第二個 ledger |
| Activities | Red-Green-Refactor、small commits、per-task self-review、unexpected failure root-cause analysis |
| Minimum artifact | Code/config/migration + tests + task state；Durable Change 只更新同一份 `tasks.md` |
...
| Fast Delivery multi-task plan、tasks mostly independent、同一 session | `subagent-driven-development` |
| Fast Delivery approved plan 交給另一 session 分批執行 | `executing-plans` |
| Durable Change with OpenSpec tasks | `/opsx:apply`；subagent 如被使用也只能協助當前 task，不得擁有另一份 plan/ledger |
| Failure/root cause unknown | 暫停 random patch，改用 `systematic-debugging` |

#### Durable Change — `tasks.md` Single Ledger + JIT Breakdown
...
- 不建立第二份 plan 或 ledger。Durable Change **不得使用 `superpowers:writing-plans`**；該 skill 會另外產生 `docs/superpowers/plans/` 文件，造成 dual SSOT。
...
| Implement | P0 Playbook；Fast Delivery 按需 `writing-plans`；Durable Change 由 `/opsx:apply` 依 `tasks.md` 執行 | 逐張 P0 Red-Green-Refactor、small commits、per-task checks | P0 increments + tests + updated ticket/task state | Per-P0 reviews pass |
...
| Plan | Fast Delivery：單一步驟用 inline plan，multi-step/high-risk/complex 用 JIT `writing-plans`；Durable Change：只用 OpenSpec `tasks.md` | Exact steps、files/interfaces、tests、commands |
...
| Spec-Driven Development | OpenSpec | Design → Plan → Implement → Validate | 需要 durable agreement/change traceability 時使用；Fast Delivery 可不加 |
...
| writing-plans |  |  | Fast Delivery per-P0 JIT only | Execution contract | Plan compliance check |
...
| executing-plans / subagent-driven-development |  |  | Read approved plan | Fast Delivery multi-task work only | Completion ledger |
...
| Fast Delivery 的單一 P0 即將執行，且 multi-step、high-risk 或 complex | `writing-plans`；簡單單一步驟 P0 可 inline |
| Durable Change 的下一個 OpenSpec task 顆粒過粗 | 在 `/opsx:apply` 中直接於 `tasks.md` 展開巢狀子項；不使用 `writing-plans` |
...
→ choose one artifact profile:
    Fast Delivery → brainstorming / Team design SSOT
    Durable Change → /opsx:propose or new/continue
→ grill-with-docs (risk-based)
→ to-tickets: P0 slices + Blocked by
→ each frontier P0: writing-plans if Fast Delivery and triggered;
                    strengthen OpenSpec tasks.md if Durable Change
→ Fast Delivery: worktree + TDD + selected execution skill
  Durable Change: isolated workspace + /opsx:apply under TDD discipline
...
→ Fast Delivery: inline plan, or writing-plans if multi-step/complex
  Durable Change: use tasks.md; expand a coarse task in place during /opsx:apply
...
→ otherwise use Fast Delivery profile
```
...
8. **Durable Change 只有一份 `tasks.md`。** `/opsx:apply` 依它取 task、打勾與交接；不得使用 `writing-plans` 或建立第二份 plan/ledger。粗 task 直接在原檔做 JIT breakdown。
...
12. **不要先替整個 Epic 建立 implementation plan。** 先拆 Features，再用 `to-tickets` 切 P0；`writing-plans` 只供 Fast Delivery 的單一 P0 即將執行時觸發。
...
- Plan stage 的分工：`to-tickets` 負責 P1 → P0；Fast Delivery 的 P0 按需用 JIT `writing-plans`；Durable Change 只用 `tasks.md` 並在 `/opsx:apply` 中就地展開粗 task。
...
- Engineer start path 只呈現兩條 delivery route：Fast Delivery 由 Superpowers end-to-end；Complex/Durable Change 由 OpenSpec 管 spec/tasks，再於 `/opsx:apply` 中逐 task 使用 Superpowers TDD/review。Discovery 只是尚未清楚時的 pre-route，不是第三條 delivery flow。
