...
> Governing Framework: `02_Framework.md` v1.8
...
| Design | 是否需要 System Design？要改成什麼、為什麼選這個方案？ | System Design when triggered / optional `superpowers:brainstorming` / `grill-with-docs` |
...
| Existing system 不熟，尚未找到真實 behavior/change boundary | 執行 manual `codebase-research`；範圍更廣時先做 `system-research` |
| 規則主要在人的經驗裡 | `grill-me` |
...
```text
system-research + codebase-research + grill-me
→ /opsx:explore
...
| Design | Triggered 時進行 System Design；需要 durable agreement 時用 `/opsx:propose`；其他 AI aids optional | System boundary/architecture delta when triggered；target behavior、contracts/data/failure、test strategy | Triggered System Design Pack/ADR + Proposal + Specs + Design | System Design Review when triggered；Feature Design approved |
...
```text
system-research + codebase-research (for brownfield)
