...
> Derived from: `02_Framework.md` v1.14 Baseline + `03_Golden_Engineering_Playbook.md` v1.12 Baseline + `04_Framework_Overview.md` v1.12 Candidate
...
進入新的 work item 時，先完整回答 Six Golden Questions：工作記在哪裡、Work Level、Archetype、Current Stage、Control Profile 與 Next Move。開始執行後通常只更新 Current Stage 與 Next Move；scope、architecture、risk 或 AI authority 改變時，才重新回答六題。
...
| Approved outcome 尚未成為可執行 slices/steps | **Plan** | P1 只有多個獨立 outcome 才用 `to-tickets`；P0 已採用 OpenSpec 就用 `tasks.md`，未採用才 inline／按需 `writing-plans` | §6–7 |
...
- Plan 不可跨層：P3/P2 做 delivery decomposition；P1 只有需要多個 P0 時才用 `to-tickets`；P0 已採用 OpenSpec 時只用 `tasks.md`，未採用時才按需用 `writing-plans`。
...
| Feature outcome that may contain one or more Sprint slices；可被整體驗收/release/control | **P1 Feature** | Research + approved Feature Design；多個 slices 才 `to-tickets`，單一 bounded slice 直接作為 P0 | Split by technical layer |
...
| Spec-Driven Development | OpenSpec；non-durable work 可用 existing ticket／design SSOT | 是否需要 durable proposal／specs／design／tasks 與 change traceability？ |
...
### 6.2 Select Only Inside the Current Stage

Six Golden Questions 與 Current Stage 已確認後，才使用下列判斷；OpenSpec、Superpowers、`to-tickets`、`writing-plans` 不是 initial routing axes。

| Current Stage | Stage-internal decision |
|---|---|
| **Research** | 依第一個未知只選一個 evidence capability |
| **Design** | 需要 durable agreement／handoff 時使用 OpenSpec；否則沿用 existing SSOT 與 Superpowers／Team equivalent |
| **Plan** | 分開判斷 delivery outcomes 與 implementation steps；見 §7 |
| **Implement** | 確認唯一 task ledger 與 execution entry，再套用 TDD／debugging／targeted checks |
| **Validate** | 建立 review／verification evidence；OpenSpec 按 risk verify／sync／archive |

是否採用 OpenSpec，只描述 Stage 內的 artifact／execution selection，不是另一個 Engineer workflow。architecture/risk trigger、required design/test/review、Human Gate 與 evidence 不因選擇改變。

如果 Current Stage 的「第一個問題」還不清楚，再依問題來源選 capability：
...
| 已決定改變 behavior，但 target design/approach 尚未 approved | `superpowers:brainstorming` 或 Team equivalent | Design 經 Human approval；結果保存在 selected design SSOT |
| Design context 必須跨 session／AI／Engineer 延續 | OpenSpec `/opsx:propose` or `new/continue` | 建立 scoped P1/P0 Change；套用相同 design quality contract，但不建立第二份 design |
...
| **Design** | 已決定新增/改變 behavior，但 target behavior/options/trade-offs 未 approved | `superpowers:brainstorming` 或 Team equivalent | Selected design SSOT | Change Gate |
| **Design** | Design/spec 已有，但可能有 contradiction/gap | `grill-with-docs` | Findings + closed decisions | Change Gate |
| **Design** | 下一個 session、AI 或 Engineer 必須接著同一份 why／what／how 工作 | `/opsx:propose` or `/opsx:new` + `/opsx:continue` | Scoped proposal/specs/design；tasks 在 Plan 補齊；不另建 duplicate design | Change Gate |
| **Plan** | Approved P1 包含多個獨立 outcome，尚未切成 Sprint-ready slices | `to-tickets` | P0 tracer bullets + acceptance + `Blocked by` | Change Gate：slices independently verifiable |
| **Plan** | 未採用 OpenSpec 的單一 P0 即將執行；multi-step/high-risk/complex | `superpowers:writing-plans` 或 Team equivalent | Exact files/interfaces/tests/commands | Change Gate：plan executable |
| **Plan** | 已採用 OpenSpec，`tasks.md` 尚未達 executable standard | 直接在同一份 `tasks.md` 補強或展開 sub-tasks | OpenSpec `tasks.md`；不另建 duplicate plan | Change Gate：plan executable |
...
flowchart TD
  ...
  A -->|P1 with multiple slices| TT["to-tickets\nP0 tracer bullets + blockers"]
  TT --> F
  A -->|P1 = one bounded slice| P0B["Treat as one P0"]
  P0B --> F
  A -->|P0| F{"On execution frontier?"}
  F -->|No| B["Resolve or wait for Blocked by"]
  F -->|Yes| O{"OpenSpec Change in use?"}
  O -->|Yes| T["Use the same tasks.md\nExpand sub-tasks in place"]
  O -->|No| J{"Multi-step, risky, or complex?"}
...
- `to-tickets` owns **conditional P1 → multiple P0 delivery decomposition**。
- OpenSpec `tasks.md` owns **implementation breakdown** when a Change is already in use；粗 task 直接在原檔展開。
- `writing-plans` owns **one P0 → Execution Layer** only when OpenSpec is not used，並採 just-in-time。
- 不替整個 Epic/Feature 預先建立巨大 implementation plan。
- Wide refactor 使用 `expand → migrate batches → contract`；每批是可驗證 P0，並標明 blocker。
- P1-scoped OpenSpec Change 如涵蓋多個 P0，`tasks.md` reference P0 IDs；tracker 保存 outcome／acceptance／`Blocked by`，OpenSpec 保存 implementation steps，不重複維護相同 tasks。
...
5. **No giant plans**：P1 只有需要多個 P0 時才 `to-tickets`；單一 bounded slice 直接走 P0 route；P0 已採用 OpenSpec 就用 `tasks.md`，未採用才按需 `writing-plans`。
...
→ each bounded P0: use OpenSpec tasks.md when adopted;
                    otherwise inline or JIT writing-plans
...
→ use OpenSpec tasks.md when adopted;
  otherwise inline plan or writing-plans if complex
→ minimal TDD fix
...
→ if a Feature contains multiple slices: to-tickets
