...
6. **Next Move 是什麼？** 執行 required engineering activity，並按未知選用 supporting skill；把 minimum information/evidence 寫回 existing workflow，再完成相稱 gate decision。
...
| Boundary、target behavior、trade-off 或 failure mode 未決 | **Design** | 依 trigger 進行 System Design 或 Feature/Detailed Design；AI aids optional | §5–6 |
...
使用方式：先用 Six Golden Questions 定位，再選 **P3–P0 column** 與目前最大未知所屬的 **Golden Stage row**；cell 左側是 required engineering activity／supporting skill，箭頭右側是 minimum output。
...
| **Design** | System Design → Product/Target System Design、ADRs、Epic/Wave boundaries；AI aids optional | System Design → Epic System Design、ADRs、Feature Map；AI aids optional | Triggered System Design；scoped OpenSpec when durable agreement is needed → Approved Feature Design/Spec | Micro-design；OpenSpec optional → Selected Approach、must-not-change、test direction |
