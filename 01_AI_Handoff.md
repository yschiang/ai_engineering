...
| 05 | `05_Decision_Tree.*` | Diagram / Markdown | Pending supplement；work/risk/stage/skill/control 判斷 |
...
1. Sponsor review `diagrams/04_Framework_Overview.md` v1.1 Candidate 的 clarity、completeness 與 one-page density。
2. 核准後產生 `05_Decision_Tree`，完整呈現 Delivery selection + Control Profile + skill/artifact/gate logic。
3. 產生 `06_Training_Presentation.pptx` 與 `07_Quick_Reference_Guide`。
...
4. Sponsor 確認 04 Overview 的 one-page density、terminology 與 diagram readability 後轉 Baseline。
...
> **Read `00_Project_Charter.md` v1.3, `01_AI_Handoff.md` v1.5, `02_Framework.md` v1.4, `03_Golden_Engineering_Playbook.md` v1.2 Baseline, and `diagrams/04_Framework_Overview.md` v1.1 Candidate. Review the Overview for one-page clarity and consistency. After approval, create `05_Decision_Tree` as a supplement that routes Work Level, P0 type, Archetype, System Design trigger, Change Risk, Execution Mode, Current Stage, next skill/action, artifact, and human gate. It must preserve P1 → P0 `to-tickets`, per-P0 JIT `writing-plans`, and Task as Execution Layer. Do not route every path to the same controls.**
...
- [ ] 03 v1.2 已是 Baseline；04 v1.1 Candidate 已建立。
- [ ] 下一步是 review 04；核准後建立 `05_Decision_Tree`，不是直接做 PowerPoint。
