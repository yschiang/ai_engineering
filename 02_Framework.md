...
未完成 equivalent mapping 時，預設採用 Department Golden default。Framework 不強迫所有 Team 使用同一工具名稱，但也不把 skill selection 退化為無標準的個人偏好。
...
`03_Golden_Engineering_Playbook.md` v1.4 Baseline、`diagrams/04_Framework_Overview.md` v1.4 Candidate、`diagrams/05_Decision_Tree.md` v1.5 Candidate 與 `presentation/06_Training_Presentation.pptx` v1.7 Baseline Candidate 已同步 Engineer mental-model correction。後續依 adoption need 產生：
