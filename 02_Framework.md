...
本 Framework 不把 AI 定義成自動寫 code 的助手，而是將它定位為參與完整工程責任的協作能力：從理解既有系統、挑戰需求與設計、產生與修改實作、建立驗證證據，到協助既有 release 與 operation activities。

Engineer-facing operating model 只有一套：

- **Golden Flow**：`Research → Design → Plan → Implement → Validate`
- **Six Golden Questions**：新 work item 進場時完成一次 initial routing；執行中通常只更新 current stage 與 next move，context 改變時再完整重判。

在每個 Golden Stage，Engineer 都使用同一組 **AI 基本功**：`Understand → Challenge → Execute → Evidence`。這是工作紀律，不是第二套 lifecycle。AI Execution Mode `Observe → Propose → Change → Act` 只回答「AI 在目前授權下可以做到哪裡」，也不是 workflow。
...
### 3.1 Golden Flow — The Single Engineer Mental Model
...
| Golden Stage | 核心問題 | 主要產出 |
|---|---|---|
| Research | 是否理解正確的 problem、current state、scope 與 options？ | Research evidence、problem/scope、unknowns、initial risk |
| Design | 應改變什麼，為什麼選這個方案，如何避免破壞系統？ | Selected design、contracts、trade-offs、failure/test direction |
| Plan | 如何形成可交付、可執行且可驗證的小步驟？ | P0 slices、dependencies/frontier、triggered JIT plan |
| Implement | 如何用 bounded、reviewable 的方式執行？ | Code、config、migration、tests、task state |
| Validate | 有什麼 fresh evidence 支持完成與 release handoff？ | Acceptance/risk evidence、review closure、release readiness |

Golden Flow 不是 Team mandatory SDLC phases。進入新的 work item 時，Engineer 先完成 Six Golden Questions 的 initial routing；進入執行後，沿用已確認的 Work Level、Archetype 與 Control Profile，通常只更新 current stage 與 next move。若 scope、architecture、risk 或 AI authority 改變，再完整重判。一個 activity 可以承載多個 Stages，一個 Stage 也可以跨 activities／Sprints。

Six Golden Questions 是每個新 work item 的 initial routing checklist：

1. **工作記在哪裡？** 哪個既有 ticket、PR、design doc、test 或 release record 是大家共同查看的地方？
2. **Work Level**：P3 Product/Program、P2 Epic、P1 Feature 或 P0 PBI/User Story？
...
5. **Control Profile**：System Criticality、Change Risk Tier、AI Execution Mode 需要多嚴謹？
6. **Next Move**：需要哪種 capability、minimum artifact／evidence 與 human gate decision？

Golden Stage 只定位「目前是哪一類工程問題」，不直接指定 tool。先找出**第一個還沒搞清楚的問題**，只選一個最適合的 capability；做到預期結果後，再判斷下一步。若結果需要交給下一個 session、AI 或 Engineer，或需要長期維護與稽核，才寫進既有 System of Record 或正式 artifact。不要因為多個 skills 都能對話或規劃，就把它們全部串成固定流程。

Tool/skill selection 採兩層判斷：先由 problem clarity、change scope/risk、handoff 與 evidence need 決定不可省略的 capability；再由 Team 依既有 workflow、tool availability、熟悉度與維護成本選擇 Golden default 或 approved equivalent。Team 習慣可以改變「怎麼做、內容放哪裡」，不能省略 required design、test、review、gate 或 evidence。每個 Team 應有一致的 default profile，避免每位 Engineer 臨時發明自己的流程。

**Tools、skills 與 artifact implementation 都是 Current Golden Stage 內的選擇，不是 initial routing dimension，也不是另一條 lifecycle。** Engineer 先完成 Six Golden Questions、定位 Current Stage 與 Next Move，之後才依該 Stage 的工程問題選擇 OpenSpec、Superpowers、`to-tickets`、`writing-plans` 或 approved equivalent。

### 3.2 AI 基本功 in Every Golden Stage

每一個 Golden Stage 都使用同一組 AI 基本功。它描述「如何把該 Stage 做好」，不是另一條 delivery lifecycle：
...
### 3.5 Engineering Responsibility Coverage

Golden Flow 加上 Team 既有 release／operation activities，必須共同覆蓋以下工程責任：

| Golden Stage / Existing Activity | Covered Responsibilities |
|---|---|
...
| Existing Release / Change / Operate | Authorize → Release → Observe → Recover / Learn |

這是一張 **Framework completeness check**，不是正式 lifecycle model，也不是 Engineer 要再執行一次的第二套流程。後面的 **6–12 Reference Appendix** 只供 governance、architecture review、quality-policy 維護或特定缺口查詢；Engineer 以 Six Golden Questions 完成 initial routing 後，日常仍只沿 Golden Flow 推進。
...
Golden Plan 區分兩種不同問題：**delivery decomposition** 判斷 approved P1 是否包含多個可獨立交付的 P0 outcomes；**implementation breakdown** 判斷目前 Change／P0 需要哪些 execution tasks。只有前者成立時才使用 `to-tickets`（或 Team equivalent）；若 P1 本身就是一個 bounded vertical slice，直接視為一個 P0。後者若已採用 OpenSpec，使用同一份 `tasks.md` 並在原檔按需展開 sub-tasks；未採用 OpenSpec 時，簡單 P0 使用 inline plan，multi-step／high-risk／complex P0 才按需使用 `writing-plans`。具體 trigger、artifact 與工具 mapping 由 `03_Golden_Engineering_Playbook.md` 管理。

### 3.6 Responsibility Ownership and Stage-Internal Capability Selection

不要先問「要跑哪一套 framework」，先確認這次工作需要哪些責任，以及每項責任由哪個既有 owner／artifact／tool 承接。

| Responsibility | Accountable Owner | 可選 Implementation | Department Rule |
|---|---|---|---|
| Direction／roles | Product、Engineering、Architecture 與 Service owners | Team workflow 或 approved equivalent | Framework 不新增 virtual organization；既有 Human owner 對 outcome、trade-off 與 production decision 負責 |
| Spec-Driven Development | Work Owner 對 behavior agreement 負責 | OpenSpec；non-durable work 可使用 existing ticket／design SSOT | OpenSpec 是 Department 的 durable change agreement default：維護 proposal／specs／design／tasks 與 change traceability |
| Engineering discipline | Engineer + Reviewer | Superpowers 或 Team equivalent | Superpowers 是 TDD-centered discipline default：涵蓋 design、planning、worktree、TDD、debugging、execution、review 與 fresh verification |
| Execution orchestration | Engineer | `/opsx:apply`、Superpowers execution skill 或 approved runner | 同一 change 只選一個 execution entry 與 task ledger；runner 不能自行改寫 approved scope |
| Evidence／approval | Accountable Human Owner | CI、PR、tests、review、OpenSpec verification 與既有 release record | Automation 提供 evidence；Human Gate 決定是否通過、返工或接受 residual risk |
...
兩者都能涵蓋部分 Design、Plan、Implement、Validate，因此混用時必須先選 artifact owner，不能因功能重疊而產生雙份 SSOT。這些判斷只在 Current Stage 內發生：

- **Research**：尚未清楚 problem、current behavior、root cause 或 options 時，只選第一個未知所需的 research、`grill-me`、`/opsx:explore` 或 debugging capability。
- **Design**：需要跨 session、AI 或 Engineer 保存 why／what／how，或 behavior agreement 必須長期維護時，才由 OpenSpec 承接 proposal／specs／design；否則使用既有 ticket／design SSOT 與 Superpowers／Team equivalent。
- **Plan — delivery decomposition**：只有 approved P1 包含多個可獨立交付的 outcome 時，才使用 `to-tickets`／Team equivalent 建立 P0；一個 bounded P1 直接當作單一 P0。
- **Plan — implementation breakdown**：OpenSpec Change 使用 `tasks.md` 作唯一 execution ledger；沒有 OpenSpec 時，簡單 P0 使用 inline plan，multi-step／high-risk／complex P0 才使用 `writing-plans`。
- **Implement／Validate**：由既有 task ledger 選定 execution entry，套用 TDD、debugging、review 與 fresh verification；automation 產生 evidence，Human Owner 決定 Gate。

> **OpenSpec Change canonical ledger rule：OpenSpec `tasks.md` 是唯一 task ledger；`/opsx:apply` 遇到粗 task 時只在原檔做 JIT 子項展開，不得使用 `writing-plans` 或另建 plan/ledger；若改變 `design.md` 的做法則使用 `/opsx:update`，並依 scope/risk 判斷是否重新通過 Change Gate。**

Team 習慣會影響 default profile、artifact placement 與 approved equivalent，但不改變 responsibility ownership，也不能降低 architecture/risk trigger、TDD/test/review、Human Gate 或 evidence 要求。
...
Framework 的 Golden Flow、Six Golden Questions、AI 基本功、artifacts 與 gates 必須嵌入 Team 已有的 backlog、design、development、PR、test、release、operation、incident 與 Change Management activities。它不建立另一套會議、文件鏈或 approval authority。
...
| Per-P0 implementation steps | P0 work item／Task child item、OpenSpec `tasks.md`、PR description or linked plan |
...
1. 盤點既有十個以上 cases。
2. 每個 case mapping 到 Work Level、Archetype、Golden Stage、AI 基本功的實際運用、Risk Tier、Execution Mode、artifacts/evidence、outcome 與 lessons learned。
...
Change Risk Tier 決定控制強度；Delivery Level 決定工作如何拆解與需要哪些 Golden/artifact contracts。兩者不得混用。
...
## 6–12. Reference Appendix — Engineering Responsibility Coverage

本 appendix 展開 completeness-check guidance，不定義執行順序，也不屬於 Engineer onboarding path。只有在設計 team policy、檢查責任缺口、處理 L2–L3 work 或需要特定 quality guidance 時才查閱；日常 execution 以 `03_Golden_Engineering_Playbook.md` 為準。
...
Use case 不建立獨立流程；全部使用相同 Golden Flow、Six Golden Questions、AI 基本功、Change Risk Tier 與 Execution Mode。
...
### 15.2 Golden and Existing-Workflow Artifact Map

| Golden Stage / Existing Activity | Minimum Artifact | L2–L3 Additional Artifact |
|---|---|---|
| Research | Understanding summary + problem/scope/acceptance | System/runtime/data model + risk assessment |
| Design | Selected approach + test/failure direction | System/Feature Design + decision record |
| Plan | P0 slices/dependencies or bounded inline/JIT plan | Integration/rollout/dependency evidence plan |
| Implement | Code/config/tests + execution state | Migration/compatibility artifacts |
| Validate | Test/review result + release handoff evidence | Non-functional/risk/readiness/rollback evidence |
| Existing Release / Operate | Release/observation/outcome record | Drill/incident/learning report |
...
|---|---|
| Project root | README、Charter、Handoff、Framework、Playbook 等 navigation／governance／SSOT artifacts |
| `guides/` | 可自行閱讀、按需查詢的 overview 與 detailed routing reference |
| `training/` | 實際存在的 training deck、assets 與 rendered review artifacts |
| `reference/` | 影響判斷、但不是 core method 的 source／mapping reference；目前管理 Golden Skill Registry |

Supporting directory 只在有實際內容時建立，不預先建立空的 `input/`、`output/`、`archive/` 或其他 placeholder。產品 repository 不需複製整套 Framework，只需保留最小 AI-readable context：system purpose/boundaries、build/test/run commands、architecture and coding rules、tests、deployment/operations references。實際檔名依既有 repository standard，不強制新增平行結構。
...
3. **Workflow**：AI 被放進真正 engineering activities，而不是旁邊的實驗工具。
...
2. 統一 mapping Work Level、Archetype、Golden Stage、AI 基本功的實際運用、Risk Tier、Execution Mode、artifacts/evidence、outcome 與 lessons learned。
...
| Unbounded Vibe Coding | 未完成 routing 就讓 AI 大量改 code，沒有 design、test discipline 或 stop condition | 新工作先完成 Six Golden Questions；Research 採 manual capability default，其他路徑採 approved default／equivalent |
...
完整 decision logic 由 `guides/05_Decision_Tree.md` 呈現；其開頭同時擔任 compact quick reference。本 Framework 保留原則與判斷維度，不複製另一套流程。
...
| Policy Level | Rule |
|---|---|
| **Required discipline** | 命中條件時不可省略：unfamiliar brownfield change 先建立 codebase evidence；behavior change 採 test-first discipline；completion claim 前執行 fresh verification |
| **Golden default** | Department 提供可直接採用的 manual capability contract 或 approved implementation，以及 required inputs/outputs、stop condition 與 gate |
| **Approved equivalent** | 成熟 Team 可替換實際 tool/skill，但必須 mapping 到相同 capability、required inputs/outputs、stop condition、gate 與 evidence quality |

未完成 equivalent mapping 時，預設採用 Department Golden default。Framework 不強迫所有 Team 使用同一工具名稱，但也不把 skill selection 退化為無標準的個人偏好。Golden default 的 source、environment、installation、invocation、status 與 fallback 由 `reference/Golden_Skill_Registry.md` 管理。
...
- 先指定 canonical home；完整 table、diagram、rule set 只維護一份，其他 artifact 使用 link 或必要的一行 guardrail。
- 採 **impact-based synchronization**：只更新 canonical artifact 與實際受影響的 derived view。README 只在 navigation 改變時更新；Overview／Decision Tree 只在 visible routing 改變時更新；Training 只在教學路徑或 audience-facing copy 改變時更新；Skill Registry 只在 capability implementation/readiness 改變時更新。
- 每次變更記錄已檢查哪些 derived artifacts 與為何 update／no-op；不得把「同步全部文件」當作預設。
- Decision Tree 開頭擔任 quick reference，不維護重複的 `07` content SSOT。
...
| Version | Change |
|---|---|
| v1.14 | Engineer-entry correction：Six Golden Questions 保持 initial routing，Golden Flow 定位 Current Stage；OpenSpec、Superpowers、`to-tickets` 與 `writing-plans` 明定為 Stage-internal capability selection。Plan 正式區分 P1→P0 delivery decomposition 與 Change／P0→tasks implementation breakdown；OpenSpec `tasks.md` 保留 JIT sub-task breakdown 與 single-ledger ownership。 |
| v1.13 | Adoption simplification：`to-tickets` 改為條件式能力，只有 P1 包含多個獨立交付 outcome 時才使用；單一 bounded P1 直接作為一個 P0，走 Superpowers／Team equivalent route。同步更新 Golden Plan、responsibility routing 與 P1/P0 boundary。 |
| v1.12 | Durable ledger correction：canonical 定義 OpenSpec `tasks.md` 為唯一 task ledger；粗 task 由 `/opsx:apply` 在原檔 JIT 展開，禁止 Durable Change 使用 `writing-plans` 或另建 plan/ledger；改變做法則走 `/opsx:update` 並依 scope/risk 判斷 Change Gate。操作細節與範例由 `03_Golden_Engineering_Playbook.md` 管理。 |
| v1.11 | Responsibility ownership correction：新增 Direction／roles、Spec-Driven Development、Engineering discipline、Execution orchestration、Evidence／approval 五項責任分工；明確由 OpenSpec 擁有 durable spec agreement/change traceability，由 Superpowers 提供 TDD-centered engineering disciplines。加入 Fast Delivery、Durable Change、Discovery First 三條 adoption-first route，以及 mixed mode 的單一 artifact owner／execution entry／task ledger 規則。 |
| v1.10 | Enforcement correction：Department rules 必須 mapping 到既有 PR／CI／tracker／GitOps chokepoint，並誠實分類 Blocking／Detective／Sampling。Rollout 只啟用 E1 Human Decision Record on PR；P0 沿用 PR approval，不增加欄位或文件。其他 detection 先定義、不啟用，待單一 Team 30-day pilot 的 friction／false-positive evidence 後再決定。Reference implementation 位於 `reference/Enforcement_E1_PR_Decision_Record.md`；不改 Golden Flow、03/04/05 或既有 gate model。 |
| v1.9 | Capability-routing correction：Golden Stage 只定位問題類型；第一個還沒搞清楚的問題決定下一個 capability；是否要交接、長期維護或稽核，決定結果要放在哪個 System of Record。客觀 engineering need 先決定 required capability，Team fit 再決定 Golden default／approved equivalent；禁止把可重疊 skills 全部串成固定流程。 |
| v1.8 | Simplicity/effectiveness correction：Sections 6–12 降為 responsibility coverage reference appendix，不進入 Engineer operating path；Change Policy 改為 canonical ownership + impact-based synchronization，禁止完整 table/diagram 跨 artifact 複製。Research rollout 採不需安裝的 manual capability defaults，candidate implementation 不再作為 active path。 |
...
| Dimension | Standard |
|---|---|
| Engineer mental model | Golden Flow：Research → Design → Plan → Implement → Validate；新工作先完成 Six Questions，執行中更新 Stage／Next Move，context 改變時完整重判 |
...
| AI 基本功 | 每個 Golden Stage 都先 Understand、Challenge，再 Execute 並留下 Evidence；不是另一套 lifecycle |
| Responsibility coverage | Understand、Define、Design、Build、Verify、Release、Operate 是 completeness check，不是 formal model |
...
| Capability routing | Six Golden Questions 先定位 Current Stage；再決定 responsibility owner 與 required capability，最後依 Team fit 選 Golden default／approved equivalent |
...
`03_Golden_Engineering_Playbook.md` v1.12 Baseline、`guides/04_Framework_Overview.md` v1.12 Candidate、`guides/05_Decision_Tree.md` v1.12 Candidate 與 `training/06_Training_Presentation_v1.16_9pages.pptx` 已同步 Six Questions entry、Golden Flow、Stage-internal capability selection、conditional P1 slicing、OpenSpec task breakdown 與 Durable single-ledger summary；single-ledger／JIT breakdown 操作細節只由 `03` 管理。Supporting adoption assets：

1. `README.md`：依角色說明 Start Here、artifact purpose、reading order 與 SSOT。
2. `reference/Golden_Skill_Registry.md`：管理 Golden default source、environment、installation、invocation、fallback 與 verification status。
3. Pilot Case Library：consolidate 既有十個以上 cases，選 3–5 canonical cases；不建立新的 pilot gate。

Decision Tree 的 opening router 是日常 quick reference；只有需要特定發布格式時才由同一內容產生 PDF／PNG，不建立獨立 `07` Markdown SSOT。所有衍生產物應以本文件為 SSOT，不新增未經確認的核心概念，也不得把 Golden Stage 畫成部門統一的固定 SDLC phase。

## Appendix C — Reference Mapping to Common SDLC Activities

本表只示範 common delivery activities 如何承載 Golden Stages、AI 基本功、artifacts 與 gates。**它不是 Department mandatory SDLC。** Team 可以合併、改名、重排或省略不適用的 activities，再把本表的 minimum information、decision responsibility 與 evidence mapping 到 local workflow。
