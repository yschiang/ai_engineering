...
Golden Plan 有兩個不同深度：先把 approved P1 Feature 切成 sprint-ready P0 vertical slices，再在單一 P0 即將執行時產生 implementation tasks。Department reference capability aliases 是 `to-tickets` 與 `writing-plans`；其 trigger、artifacts 與工具 mapping 由 `03_Golden_Engineering_Playbook.md` 管理。

### 3.6 Responsibility Ownership and Lifecycle Selection
...
| Spec-Driven Development | Work Owner 對 behavior agreement 負責 | OpenSpec；Fast Delivery 可使用 Superpowers design/plan 或 Team SSOT | OpenSpec 是 Department 的 durable change agreement default：維護 proposal／specs／design／tasks 與 change traceability |
...
兩者都能涵蓋部分 Design、Plan、Implement、Validate，因此混用時必須先選 artifact owner，不能因功能重疊而產生雙份 SSOT：

- **Fast Delivery**：對已清楚、bounded 的 P0／P1，可由 Superpowers／Team equivalent 完成 short design、plan、TDD、review 與 verification；ticket／design／PR chain 是 SSOT，不必為了形式加入 OpenSpec。
- **Durable Change**：當 why／what／how／tasks 必須跨 session、AI 或 Engineer 延續，或 behavior agreement 需要長期維護時，由 OpenSpec 擁有 proposal／specs／design／tasks；`/opsx:apply` 依該 agreement 實作，並選用 Superpowers 的 TDD、debugging、review 與 verification disciplines；不建立第二份 design／plan。
- **Discovery pre-route**：尚未清楚 problem、current behavior、root cause 或 options 時，只選第一個未知所需的 research、`grill-me`、`/opsx:explore` 或 debugging capability；方向清楚後再選 Fast Delivery 或 Durable Change。它不是第三條 delivery flow。

> **Durable Change canonical ledger rule：OpenSpec `tasks.md` 是唯一 task ledger；`/opsx:apply` 遇到粗 task 時只在原檔做 JIT 子項展開，不得使用 `writing-plans` 或另建 plan/ledger；若改變 `design.md` 的做法則使用 `/opsx:update`，並依 scope/risk 判斷是否重新通過 Change Gate。**
...
| Capability routing | 先決定 responsibility owner：OpenSpec 管 durable SDD/change traceability，Superpowers 管 TDD-centered engineering disciplines；再依客觀 need 與 Team fit 選 route/equivalent |
...
`03_Golden_Engineering_Playbook.md` v1.10 Baseline、`guides/04_Framework_Overview.md` v1.10 Candidate、`guides/05_Decision_Tree.md` v1.10 Candidate 與 `training/06_Training_Presentation.pptx` v1.14 Baseline Candidate 已同步 responsibility ownership、capability routing、Durable single-ledger summary 與 role-based navigation；Training P3 專講 responsibility ownership，P4 專講 Golden Flow／Stage／Next Move，避免重複。single-ledger/JIT breakdown 操作細節只由 `03` Implement stage 管理。Supporting adoption assets：
