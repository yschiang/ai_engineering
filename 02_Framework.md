...
- **Six Golden Questions**：用來定位 current work location、work level、archetype、current stage、control profile 與 next move。
...
Golden Flow 不是 Team mandatory SDLC phases。Engineer 用 Six Golden Questions 找到目前位置，從第一個不能可靠回答的 Golden question 開始；一個 activity 可以承載多個 Stages，一個 Stage 也可以跨 activities／Sprints。
...
這是一張 **Framework completeness check**，不是正式 lifecycle model，也不是 Engineer 要再執行一次的第二套流程。Sections 6–12 展開各 responsibility area 的 quality guidance；Engineer 日常仍只使用 Golden Flow + Six Golden Questions。
...
## 6. Responsibility Coverage — Understand
...
## 7. Responsibility Coverage — Define
...
## 8. Responsibility Coverage — Design
...
## 9. Responsibility Coverage — Build
...
## 10. Responsibility Coverage — Verify
...
## 11. Existing SDLC Responsibility — Release
...
## 12. Existing SDLC Responsibility — Operate and Learn
...
| Unbounded Vibe Coding | 未定位 Work Level／Stage／risk 就讓 AI 大量改 code，沒有 design、test discipline 或 stop condition | 用 Six Golden Questions 定位；直接採 Golden default skill/path，直到 Team equivalent 完成等價 mapping |
...
| **Golden default** | Department 提供已驗證、可直接採用的 stage-to-skill path，以及 required inputs/outputs、stop condition 與 gate |
...
- 同步更新 Root README、Overview、Decision Tree、Training Presentation 與 Golden Skill Registry；Decision Tree 開頭擔任 quick reference，不維護重複的 `07` content SSOT。
...
| Dimension | Standard |
|---|---|
| Engineer mental model | Golden Flow：Research → Design → Plan → Implement → Validate；用 Six Golden Questions 定位與 routing |
...
`03_Golden_Engineering_Playbook.md` v1.5 Baseline、`guides/04_Framework_Overview.md` v1.5 Candidate、`guides/05_Decision_Tree.md` v1.6 Candidate 與 `training/06_Training_Presentation.pptx` v1.10 Baseline Candidate 已同步 Engineer mental-model correction與 role-based navigation。Supporting adoption assets：
