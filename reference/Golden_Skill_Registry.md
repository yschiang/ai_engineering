...
| `codebase-research` | Reference implementation / **Candidate mapping** | Method reference：[HumanLayer `research_codebase.md` pinned source](https://github.com/humanlayer/humanlayer/blob/c2c31d656807cad9d389acfcbc19002733958218/.claude/commands/research_codebase.md)；portable snapshot：[NeverSight mirror pinned source](https://github.com/NeverSight/learn-skills.dev/blob/a446ce88373ee3784f04ce8fb6f3f2326230a525/data/skills-md/ferueda/agent-skills/research-codebase/SKILL.md) surfaced by this [LobeHub listing](https://lobehub.com/skills/neversight-skills_feed-research-codebase)。LobeHub/NeverSight 是 discovery mirror，不是 source SSOT；原 `ferueda/agent-skills` path 已移除 | No approved install；不要從 marketplace 直接安裝，直到 Team review、vendor/pin 並驗證 supported environments | Use the environment's approved repository-research capability；exact invocation 尚未核准 | As-is Code Map／Research Brief、entry points、call/data flow、existing patterns、file/line evidence 與 change boundary；回答 research question 後停止，不主動提出改進 | Read-only inspect code、tests and history；先讀直接指定的 files，再依需要分解 research；引用 files/symbols 並產生相同 minimum output，不強制固定資料夾或 parallel agents |
...
1. Review the HumanLayer method reference and pinned NeverSight marketplace snapshot；若採用，建立 Department-owned reviewed copy、記錄 attribution/license、pin commit，並逐一驗證 environments，之後才能把 `codebase-research` 稱為 installable Golden default。
